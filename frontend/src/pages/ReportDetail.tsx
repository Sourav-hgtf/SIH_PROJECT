...
export function ReportDetailPage() {
  ...
  // Task 2: Human-in-the-Loop SIF Labelling Workflow state
  const [selectedLabel, setSelectedLabel] = useState<"SIF" | "NON_SIF" | "UNCERTAIN">("SIF");
  const [labelReason, setLabelReason] = useState("High energy source present without adequate barrier");
  const [labelCustomReason, setLabelCustomReason] = useState("");
  const [labelNotes, setLabelNotes] = useState("");
  const [isSubmittingLabel, setIsSubmittingLabel] = useState(false);

  const user = getStoredUser();
  const canReview = user?.role === "analyst" || user?.role === "site_manager" || user?.role === "admin" || user?.role === "leadership";
  ...
  async function handleLabelReviewSubmit(e: FormEvent) {
    e.preventDefault();
    if (!id) return;
    const finalReason = labelReason === "Other" ? labelCustomReason : labelReason;
    if (!finalReason.trim()) {
      setError("A documented reason is mandatory to submit a SIF label review.");
      return;
    }
    setError("");
    setMessage("");
    ...
    try {
      await api.submitLabelReview(id, {
        label: selectedLabel,
        reason: finalReason.trim(),
        notes: labelNotes.trim() || undefined,
      });
      setMessage("Human SIF review submitted successfully. Review history and consensus status updated.");
      setLabelNotes("");
      setLabelCustomReason("");
      reload();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to submit SIF review");
    } finally {
            ...
            <section className="rounded-xl border-2 border-cyan-edge/40 bg-white p-5 shadow-card space-y-4">
              <div className="border-b border-border pb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold text-ink">4. Human-Validated SIF Labelling & Review</h2>
                </div>
                <p className="text-[11px] text-warm mt-0.5">
                  Defensible multi-reviewer workflow with consensus policies. AI predictions are suggestions only and never used as independent ground truth.
                </p>
              </div>

              {/* Task 2: Human-in-the-Loop SIF Labelling Workflow Card */}
              <div className="rounded-xl border border-indigo-200 bg-gradient-to-b from-indigo-50/50 to-white p-4 space-y-4 shadow-sm">
                <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
                  <div className="flex items-center gap-2">
                    <span className="rounded bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider">
                      Ground Truth Labelling
                    </span>
                    <h3 className="text-xs font-bold text-ink">Human-in-the-Loop SIF Determination</h3>
                  </div>
                  {/* Validation Status Badge */}
                  {report.validation_status === "VALIDATED" ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 border border-amber-300 px-2.5 py-0.5 text-[11px] font-bold text-amber-900 shadow-sm">
                      ★ Gold Label: {report.validated_label}
                    </span>
                  ) : report.validation_status === "DISAGREEMENT" ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 border border-rose-300 px-2.5 py-0.5 text-[11px] font-bold text-rose-800">
                      Disagreement (Senior HSE Required)
                    </span>
                  ) : report.validation_status === "PENDING_CONSENSUS" ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 border border-blue-300 px-2.5 py-0.5 text-[11px] font-semibold text-blue-800">
                      Pending 2nd Review (Consensus Policy)
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 border border-slate-300 px-2.5 py-0.5 text-[11px] font-semibold text-slate-700">
                      Unlabeled
                    </span>
                  )}
                </div>

                {/* Workflow Sequence: Incident -> AI Prediction -> Evidence -> Human Reviewer -> Label -> Reason -> Submit */}
                <div className="flex items-center justify-between text-[10px] text-warm bg-white border border-slate-200 rounded-lg p-2.5 overflow-x-auto gap-1">
                  <span className="font-bold text-ink">Incident</span>
                  <span>→</span>
                  <span className="font-semibold text-slate-600">AI Prediction ({report.predicted_sif ? "SIF" : "Non-SIF"})</span>
                  <span>→</span>
                  <span className="font-semibold text-slate-600">Evidence</span>
                  <span>→</span>
                  <span className="font-bold text-indigo-700">Human Reviewer</span>
                  <span>→</span>
                  <span className="font-bold text-emerald-700">SIF / NON-SIF / UNCERTAIN</span>
                  <span>→</span>
                  <span className="font-semibold text-slate-600">Reason</span>
                  <span>→</span>
                  <span className="font-bold text-indigo-900">Consensus Gold</span>
                </div>

                {/* Ground-Truth Separation Truth Matrix */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                  <div className="rounded bg-white border border-slate-200 p-2">
                    <span className="text-slate-500 block text-[10px] uppercase font-bold">AI Suggestion</span>
                    <strong className={report.predicted_sif ? "text-rose-600 font-bold" : "text-emerald-700 font-bold"}>
                      {report.predicted_sif == null ? "None" : report.predicted_sif ? "SIF Potential" : "Non-SIF"}
                    </strong>
                    <span className="block text-[9px] text-slate-400">Advisory only</span>
                  </div>
                  <div className="rounded bg-white border border-slate-200 p-2">
                    <span className="text-slate-500 block text-[10px] uppercase font-bold">Human Label</span>
                    <strong className="text-indigo-700 font-bold">{report.human_label || "UNLABELED"}</strong>
                    <span className="block text-[9px] text-slate-400">Latest reviewer</span>
                  </div>
                  <div className="rounded bg-white border border-slate-200 p-2">
                    <span className="text-slate-500 block text-[10px] uppercase font-bold">Gold Validated</span>
                    <strong className={report.validated_label ? "text-amber-700 font-bold" : "text-slate-500"}>
                      {report.validated_label || "Pending"}
                    </strong>
                    <span className="block text-[9px] text-slate-400">ML training input</span>
                  </div>
                  <div className="rounded bg-white border border-slate-200 p-2">
                    <span className="text-slate-500 block text-[10px] uppercase font-bold">Label Source</span>
                    <strong className="text-slate-700 font-semibold">{report.label_source || "UNLABELED"}</strong>
                    <span className="block text-[9px] text-slate-400">Auditable origin</span>
                  </div>
                </div>

                {/* Reviewer Action Form */}
                <form onSubmit={handleLabelReviewSubmit} className="space-y-3 bg-white border border-indigo-100 rounded-lg p-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-ink">Submit Human Label:</span>
                    <span className="text-warm text-[11px]">
                      Reviewer: <strong>{user?.username}</strong> ({user?.role})
                    </span>
                  </div>

                  {/* 3-Way SIF / NON_SIF / UNCERTAIN Selector */}
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      type="button"
                      onClick={() => setSelectedLabel("SIF")}
                      className={`rounded-lg border px-3 py-2 text-xs font-bold transition flex flex-col items-center justify-center ${
                        selectedLabel === "SIF"
                          ? "border-rose-500 bg-rose-500 text-white shadow"
                          : "border-slate-200 bg-slate-50 text-slate-700 hover:border-rose-300 hover:bg-rose-50/50"
                      }`}
                    >
                      <span>SIF</span>
                      <span className="text-[10px] font-normal opacity-90">Fatal potential</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => setSelectedLabel("NON_SIF")}
                      className={`rounded-lg border px-3 py-2 text-xs font-bold transition flex flex-col items-center justify-center ${
                        selectedLabel === "NON_SIF"
                          ? "border-emerald-600 bg-emerald-600 text-white shadow"
                          : "border-slate-200 bg-slate-50 text-slate-700 hover:border-emerald-300 hover:bg-emerald-50/50"
                      }`}
                    >
                      <span>NON-SIF</span>
                      <span className="text-[10px] font-normal opacity-90">Controlled risk</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <span>UNCERTAIN</span>
                      <span className="text-[10px] font-normal opacity-90">Need more info</span>
                    </button>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] font-semibold text-ink flex items-center justify-between">
                      <span>Reason for Decision (Mandatory):</span>
                      <span className="text-rose-500 text-[10px] font-normal">*Required by audit policy</span>
                    </label>
                    <select
                      className="w-full rounded border border-slate-300 bg-white p-2 text-xs text-ink focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                      value={labelReason}
                      onChange={(e) => setLabelReason(e.target.value)}
                    >
                      <option value="High energy source present without adequate barrier">High energy source present without adequate barrier</option>
                      <option value="Credible fatal exposure scenario identified">Credible fatal exposure scenario identified</option>
                      <option value="Near-miss with release of fatal energy">Near-miss with release of fatal energy</option>
                      <option value="Controlled energy; barriers functioned as intended">Controlled energy; barriers functioned as intended</option>
                      <option value="Low energy incident; no fatal potential">Low energy incident; no fatal potential</option>
                      <option value="Insufficient narrative details to make conclusive determination">Insufficient narrative details to make conclusive determination</option>
                      <option value="Context misunderstood by automated heuristic">Context misunderstood by automated heuristic</option>
                  ...
                  {labelReason === "Other" && (
                    <input
                      type="text"
                      className="w-full rounded border border-slate-300 bg-white p-2 text-xs text-ink"
                      placeholder="Specify custom reason..."
                      value={labelCustomReason}
                      onChange={(e) => setLabelCustomReason(e.target.value)}
                      required
                    />
                  )}
                  ...
                  {/* Notes */}
                  <div className="space-y-1">
                    <label className="text-[11px] font-semibold text-ink">Reviewer Notes (Optional):</label>
                    <textarea
                      className="w-full rounded border border-slate-300 bg-white p-2 text-xs text-ink outline-none focus:ring-1 focus:ring-indigo-500"
                      rows={2}
                      placeholder="Add any specific operational details, barrier condition notes, or peer review comments..."
                      value={labelNotes}
                  ...
                  <button
                    ...
                    className="w-full rounded-md bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-50 transition shadow"
                  >
                    {isSubmittingLabel ? "Submitting Review..." : "Submit Human Review (Evaluates Consensus Policy)"}
                  </button>
                </form>

                {/* Immutable Review History List */}
                {report.label_reviews && report.label_reviews.length > 0 && (
                  <div className="space-y-2 border-t border-slate-200 pt-3">
                    <div className="flex items-center justify-between text-xs font-semibold text-ink">
                      <span>Review History (Immutable Audit Trail)</span>
                      <span className="text-[11px] text-warm">{report.label_reviews.length} review(s) recorded</span>
                    </div>
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {report.label_reviews.map((rev) => (
                        <div key={rev.id} className="rounded border border-slate-200 bg-slate-50 p-2 text-xs space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-ink">
                              v{rev.review_version} • {rev.reviewer_username || rev.reviewer_id} ({rev.reviewer_role})
                            </span>
                            <span
